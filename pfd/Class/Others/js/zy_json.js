...
(function(g, h) {
	...
	var k = (function() {
		...
		}, _jQuery = g.jQuery, _$ = g.U, rootjQuery, quickExpr = /^(?:[^<]*(<[\w\W]+>)[^>]*$|#([\w\-]*)$)/, toString = Object.prototype.toString;
		e.fn = e.prototype = {
			constructor : e,
			init : function(a, b, c) {
				var d, elem, ret, doc;
		...
		getJSON : function(a, b,c,d,e,f,g) {
			...
			if(window.location.href.substring(0,7).toLowerCase()=="file://")
			{
				if(a.substring(0,4).toLowerCase()=="http")
				{
					...
					{
					    //alert("uexXmlHttpMgr====="+result);
					    if(result == 'error:413'){
					        //$alert("图片过大，请重新上传");
					        uexWindow.toast('0','5', '图片过大，请重新上传','2000');
					        var obj = zy_json_session[_opid];
					        var img = obj.xinxi[1];
					        var aImg = document.getElementsByTagName("img");
					        for(var i=0;i<aImg.length;i++){
					            
					            if(aImg[i].src == img){
					                aImg[i].parentNode.removeChild(aImg[i]);
					            }
					        }
                    ...
                    zy_json_session[zy_json_opid].xinxi=f;
					//uexXmlHttpMgr.setHeaders(zy_json_opid, {"Content-type":"multipart/form-data","Cache-Control":"no-cache","Accept":"*/*"});
					uexXmlHttpMgr.open(zy_json_opid, !e?"GET":e, a,60000);
					uexXmlHttpMgr.setHeaders(zy_json_opid,{"X-Auth-Token":getLocVal('token')});
					if(f)
					{
						for(var it in f)
						{
